...
-- CreateTable
CREATE TABLE `assosiation_utilisateur_evenement` (
    `Column1` INTEGER NOT NULL AUTO_INCREMENT,
    `idUtilisateur` INTEGER NULL,
    `idEvent` INTEGER NULL,
    `isAdmin` BOOLEAN NULL,

    INDEX `assosiation_utilisateur_evenement_evenement_FK`(`idEvent`),
    INDEX `assosiation_utilisateur_evenement_utilisateur_FK`(`idUtilisateur`),
    PRIMARY KEY (`Column1`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `assosiation_utilisateur_evenement` ADD CONSTRAINT `assosiation_utilisateur_evenement_evenement_FK` FOREIGN KEY (`idEvent`) REFERENCES `evenement`(`idEvent`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `assosiation_utilisateur_evenement` ADD CONSTRAINT `assosiation_utilisateur_evenement_utilisateur_FK` FOREIGN KEY (`idUtilisateur`) REFERENCES `utilisateur`(`idUtilisateur`) ON DELETE CASCADE ON UPDATE CASCADE;
